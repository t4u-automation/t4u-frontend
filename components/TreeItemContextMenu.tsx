...
export default function TreeItemContextMenu({
  ...
  const handleMenuClick = (e: React.MouseEvent) => {
    ...
    if (buttonRef.current) {
      ...
      setMenuPosition({
        top: rect.bottom + 4,
        left: rect.right - 224, // 224px = menu width (56 * 4)
      });
    }
  ...
  const getDeleteMessage = () => {
    if (type === "feature") {
      return `Are you sure you want to delete this feature "${itemName}"? This will also delete all stories and test cases within it. This action cannot be undone.`;
    } else if (type === "story") {
      return `Are you sure you want to delete this story "${itemName}"? This will also delete all test cases within it. This action cannot be undone.`;
    } else {
      return `Are you sure you want to delete this test case "${itemName}"? This action cannot be undone.`;
    }
  };
  ...
  const menuItems = [
    ...
    {
      label: type === "testcase" ? "Clone test case" : type === "story" ? "Clone story" : "Clone feature",
      icon: Copy,
      onClick: onClone,
      show: true,
    },
  ...
  const itemType = type === "testcase" ? "test case" : type;
  ...
  return (
    <>
      ...
      {/* Render menu via portal to escape scrollable container */}
