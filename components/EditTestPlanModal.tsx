...
import { Feature, Story, TestCase, TestPlan } from "@/types";

interface EditTestPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (testPlanId: string, name: string, testCaseIds: string[], description?: string) => Promise<void>;
...
export default function EditTestPlanModal({
  ...
  useEffect(() => {
    if (testPlan && isOpen) {
      ...
      setSelectedTestCaseIds(new Set(testPlan.test_case_ids));
    }
  }, [testPlan, isOpen]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !testPlan) return;

    try {
      setIsSubmitting(true);
      await onSubmit(testPlan.id, name.trim(), Array.from(selectedTestCaseIds), description.trim() || undefined);
      handleClose();
    } catch (error) {
      console.error("[EditTestPlanModal] Error updating test plan:", error);
    } finally {
